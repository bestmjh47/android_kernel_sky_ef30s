...
/*
 - apns.xml에 기본적인 APN에 대한 정보 입력
*/
#define FEATURE_SKY_DS_APNS
...
/* 
*/
#define FEATURE_SKY_DS_WIFI_BUG_FIX_ENABLE_AFTER_BOOTING
...
/************************************************************************************************************************
**     3. CTS / PortBridge / DUN / Log /vpn / BUILD
************************************************************************************************************************/
...
/*
 ...
 - LINUX/android/kernel/arch/arm/config/EF30S_msm7630-perf_defconfig 파일(파일명은 모델과 칩셋에 따라 달라짐
*/
#define FEATURE_SKY_DS_CTS_LOCALHOST

/*
 - cust_sky_ds.h 헤더 파일을 include
 - 위 파일을 include하는 파일은 모델마다 달라지기도 함.(ex. CUST_SKY.h or CUST_PANTECH.h 등등)
*/
#define FEATURE_SKY_DS_CUST_SKY_DS
...
/*
 - Roaming시 APN 변경 (web.sktelecom.com -> roaming.sktelecom.com)
   -> 방안 1, Apn List에 추가 하고 Roaming 지역에 진입하면 해당 APN을 Preference로 변경
      : PDP Reject시 Error Cause에 따라 web.sktelecom.com으로 시도 가능성이 있음
       혹은 국내에서 roaming.sktelecom.com으로 시도할 가능성이 있음
       (Android Plaform에서 Cause에 따라 자동으로 Apn List를 순차적으로 시도 하도록 되어 있음)
   -> 방안 2, getNextApn()에서 name field가 "SKTelecom" 이고 로밍 지역이면 ApnSetting.apn을 변경
      : APN 관리 메뉴를 User영역에서는 삭제하고 테스트 메뉴에만 적용할 예정이고
        name field만 SKTelecom으로만 두지 않으면 다른 APN 테스트가 가능하므로 타당해 보임 
 - 방안 2 적용        
*/
...
/*
 ...
 - reconnect시 IPv6 초기값은 DISCONNECTED로 수정. IPv6가 connecting으로 유지되는 문제 수정.
*/
...
/*
 - 3G Data 시도중 Toast Popup 출력 하도록 수정 
*/
...
*/
#define FEATURE_SKT_DS_VOIP_BLOCK_WIFI_CUT_DOWN

/*
- 전원 Off 시 3초간 block하면서 voip deregistering시도
- user가 wifi off 시 3초간 blocking하면서 voip deregistering시도
*/
#define FEATURE_SKT_DS_VOIP_USER_POWER_OFF_HANDLE

/*
- Airplane mode진입 시 3초간 block하면서 voip deregistering시도
*/
#define FEATURE_SKT_DS_VOIP_USER_AIRPLANE_MODE_HANDLE
