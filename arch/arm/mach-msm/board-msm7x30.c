...
#define MSM_PMEM_SF_SIZE	0x2300000
// james.kim@x3industries increase pmem
// #define MSM_FB_SIZE		0x600000
// #define MSM_PMEM_ADSP_SIZE      0x1800000
#define MSM_FB_SIZE		0x700000
...
void sky_snddev_voip_speaker_pamp_on(void)
{
	snd_extamp_api_SetDevice(1, SND_SKY_DEVICE_VOIP_SPEAKER_RX);
}

void sky_snddev_voip_speaker_pamp_off(void)
{
	snd_extamp_api_SetDevice(0, SND_SKY_DEVICE_VOIP_SPEAKER_RX);
}

void sky_snddev_voip_hsed_pamp_on(void)
{
	int rc;
	snddev_vreg_ncp = vreg_get(NULL, "ncp");
	if (IS_ERR(snddev_vreg_ncp)) {
...
void sky_snddev_voip_hsed_pamp_off(void)
{
	int rc;
	if (IS_ERR(snddev_vreg_ncp)) {
		pr_err("%s: vreg_get(%s) failed (%ld)\n",
