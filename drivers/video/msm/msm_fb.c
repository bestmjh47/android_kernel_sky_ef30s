...
#ifdef FEATURE_SKY_CHG_LOGO //2010.10.22 zion90 add. power off charging in display
extern uint32_t sky_sys_rst_Get_ChargingMode(void);
#define BATT_CHARGING_IN "/image/poweroff_icon_in.rle"
#endif
...
// 20110209, kkcho_temp,  EF30S의 silent_reset의 경우, 타모델과 달리 mArm->aArm 진행시 FB를 유지 할수 없는 문제가  있는 상태임.
// 관련 문제가 해결되기  //까진 해당 부분에서 sky_image를 display 함.
#if defined(FEATURE_AARM_RELEASE_MODE)			
  if(sky_sys_rst_is_silent_boot_mode()) {
      if (!load_565rle_image(INIT_IMAGE_FILE)) ;   /* Flip buffer */
       printk(KERN_DEBUG "load_565rle_image. loadbar ratio 100 \n");
      sky_lcdc_display_loadingbar(100);           
   }
  else
  {
#endif
        ...
        {
          load_565rle_image(BATT_CHARGING_IN); 
        }
        else
		{
...
static int msm_fb_open(struct fb_info *info, int user)
{
	...
	// novapex_hsj_20110808__draw image(battery charging)
	if ( sky_sys_rst_Get_ChargingMode()) {
		load_565rle_image(BATT_CHARGING_IN); 
	}
...
//해당 option을 사용하지 않을때만 처리하면 됨..   
	...
	// novapex_hsj_20110808__draw image(battery charging)
	if (!mfd->ref_cnt && !sky_sys_rst_Get_ChargingMode()) {
		if ((ret =
...
#ifdef F_SKYDISP_LAST_FB_CONVERT_xRGB8888_TO_RGB565
          fb_offset = info->var.xoffset * (info->var.bits_per_pixel >> 3) +
...
#ifdef F_SKYDISP_LAST_FB_CONVERT_xRGB8888_TO_RGB565
unsigned int msm_fb_get_offset(void)
{
    return fb_offset;
}
EXPORT_SYMBOL(msm_fb_get_offset);

void msm_fb_set_offset(unsigned int offset)
{
    struct fb_info *info;
    uint8 *buf;

    info = fbi_list[0];
    buf = (uint8 *)info->fix.smem_start;
    buf += offset;
    mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
    MDP_OUTP(MDP_BASE + 0x90008, (uint32)buf);	
    outpdw(MDP_BASE + 0x40000/*MDP4_RGB_BASE*/ + 0x0010, (uint32)buf);
    mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_OFF, FALSE);
}
EXPORT_SYMBOL(msm_fb_set_offset);

void msm_fb_stop_display(void)
{
    msm_fb_ops.fb_pan_display = NULL;
    msm_fb_ops.fb_ioctl = NULL;
}
EXPORT_SYMBOL(msm_fb_stop_display);
#endif
